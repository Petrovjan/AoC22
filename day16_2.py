    ...
    for v in range(len(explored)):
        ...
        for poss in distances.keys():
            if poss[1] not in explored[v][1] and explored[v][1][-1] == poss[0]: #nevracet se na navstivene misto
                if explored[v][0] + distances[poss] + 1 <= 26:
...
for path in completed:
    ...
    for i in range(1, len(path[1])):
        ...
        sc = int(datDict[path[1][i]][0]) * (26 - curtime)
        curscore += sc
    scores.append([curscore, path[1]])
...
bestscore = maxscore
for hu in range(len(sScores)-1, -1, -1):
    print(hu)
    start = "AA"
    explored = [[0, [start]]]  # [[2, [AA, BB]],[3, [AA, DD]]]
    completed = []
    while True:
        ...
        for v in range(len(explored)):
            added = False
            for poss in distances.keys():
                if poss[1] not in explored[v][1] and explored[v][1][-1] == poss[0] and poss[1] not in sScores[hu][1]:  # nevracet se na navstivene misto
                    if explored[v][0] + distances[poss] + 1 <= 26:
                        newex.append(
                            [explored[v][0] + distances[poss] + 1, explored[v][1] + [poss[1]]])  # pridat cas k otevreni
                        added = True
            if not added:
                completed.append(explored[v])
        explored = newex[:]
    ...
    for path in completed:
        ...
        if curscore > bestel:
            bestel = curscore
    partres = bestel + sScores[hu][0]
    if partres > bestscore:
        bestscore = partres
        print(bestscore)
        break
print("part2: ",bestscore)
